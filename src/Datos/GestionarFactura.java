...
//        String sql = "SELECT * FROM factura WHERE fac_codigo=" + cod + "";
    ...
    public static List listFacturas(String fecha) {
        ...
        sql.append("`p-ventabd_v_c`.forma_pago.descripcion_pago,");
        sql.append("`p-ventabd_v_c`.factura.boleta_nro,");
        sql.append("`p-ventabd_v_c`.factura.monto_pago,");
        sql.append("`p-ventabd_v_c`.factura.vuelto_pago");
        sql.append(" FROM `p-ventabd_v_c`.factura INNER JOIN `p-ventabd_v_c`.vendedor INNER JOIN `bd_v_c`.clientes INNER JOIN `bd_v_c`.timbrado INNER JOIN `bd_v_c`.puntoemision INNER JOIN `p-ventabd_v_c`.forma_pago");
        ...
        sql.append(" AND `p-ventabd_v_c`.factura.f_pago = `p-ventabd_v_c`.forma_pago.idf_pago");
